...
class ServiceController extends Controller
{
    ...
    public function __construct(JsonResponse $response, private ServiceRepository $repository)
    {
    ...
    public function store(StoreServiceRequest $request): HttpJsonResponse
    {
        if (($service = $this->repository->store($request->parent_id, $request->title))) {
            return $this->onOk((new FileUploaderController($this->imageStorage))->uploadImage($service, $request, 'image', 'image'));
        }
