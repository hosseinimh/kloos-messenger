...
import { storagePath, vendorsPath } from "../../constants";
import { general, postsPage } from "../../constants/strings";
...
const TreeNodeService = ({
    ...
    const handlePosts = () => !layoutState?.loading && onPosts(node?.id);
    ...
    return (
        ...
        >
            ...
            {node?.image && (
                <img
                    src={`${storagePath}/services/images/${node?.image}`}
                    className="node-icon"
                />
            )}
            {!node?.image && (
            ...
            {hover && (
                <>
                    ...
                    <div
                        className="action-button mr-2"
                        onClick={handlePosts}
                        title={postsPage._title}
                    >
                        <svg className="action-icon">
                            <use
                                xlinkHref={`${vendorsPath}/@coreui/icons/svg/linear.svg#cil-chat-square`}
                            ></use>
                        </svg>
                    </div>
                </>
